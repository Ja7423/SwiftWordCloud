...
class ViewController: UIViewController {
     ...
     */
    let cloudFontName : String = "PingFangTC-Medium"
    /**
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        notificationCenter.addObserver(self, selector: #selector(contentSizeCategoryDidChange(_:)), name: UIContentSizeCategory.didChangeNotification, object: nil)
    }
...
extension ViewController : CloudLayoutOperationDelegate {
    ...
    func insertWord(_ word: String, pointSize: CGFloat, color: Int, center: CGPoint, vertical: Bool) {
        ...
        if (vertical == true) {
            wordLabel.transform = CGAffineTransform(rotationAngle: CGFloat(Double.pi / 2))
        }
